...
async function addAdmin(username, email, name, password, operation, type) {
    try {
        const response = await fetch('http://127.0.0.1:5001/rd-year-project-1f41d/europe-west2/userOps', {
            method: 'POST',
            headers: {
...
async function verifyAdmin_test() {
    ...
    let verdict = await addAdmin('user4', 'user4@gmail.com', 'first4 last4', 'user4_password!', 'create', 'admin')
    console.log(verdict)
}
